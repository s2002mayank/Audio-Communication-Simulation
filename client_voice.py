...
server=input("enter IP address of server: ")
...
SERVER_IP = server  # assigned server ip address
SERVER_PORT = 7575
running = True
...
# player <recipient> will wait until this many bytes are available in the buffer before playing
...
key = b'NetworkingWorkshopProjectVoiceOv'
...
def split_send_bytes(s, inp):
    data_len = (len(inp))
    if data_len == 0:
        print('ERROR: trying to send 0 bytes')
        return
...
# 4 separate threads to handle recording/transmission and Reception/playback of the audio files
# start recording and keep sending data infinitely
